...
module Ratchet
  class ConstantDiscoveryTest < Minitest::Test
    ...
    test "discovers simple constant" do
      ...
      assert_equal("components/sales/app/models/order.rb", constant.location.to_s)
    end
    ...
    test "discovers nested constant" do
      ...
      assert_equal("components/sales/app/models/sales/order.rb", constant.location.to_s)

      constant = discovery.context_for("HasTimeline")
      assert_equal("::HasTimeline", constant.name)
      assert_equal("components/timeline/app/models/concerns/has_timeline.rb", constant.location.to_s)
    end
    ...
    test "discovers constant that is partially qualified inferring its full qualification" do
      use_template(:skeleton)
    ...
    test "discovers constant that is both partially qualified and does not have its own file" do
      use_template(:skeleton)
      constant = discovery.context_for(
    ...
    test "discovers constant in root dir with non-default namespace" do
      ...
      assert_equal("components/sales/app/internal/special.rb", constant.location.to_s)
    end
