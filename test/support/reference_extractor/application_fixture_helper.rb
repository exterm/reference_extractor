...
module ReferenceExtractor
  module ApplicationFixtureHelper
    TEMP_FIXTURE_DIR = File.join(ENV.fetch("RUNNER_TEMP", "/tmp"), "reference-extractor-test", "fixtures").to_s
    ...
    def config
      @config ||= Configuration.from_path(app_dir)
    end
    ...
    def fixture_path
      File.join(TEMP_FIXTURE_DIR, "#{@template}-#{name}-#{Time.now.strftime("%Y_%m_%d_%H_%M_%S")}")
    end
  end
end
